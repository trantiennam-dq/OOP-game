...
- **Vấn đề:** Lớp `BasicMatchLogic` sử dụng cấu trúc `if-else` cứng để xét nếu là kích cỡ = 4 thì tạo kẹo sọc ngang/dọc nổ hết hàng ngang/dọc , nếu >= 5 thì tạo kẹo gói nổ 3x3 xung quanh. Bạn phải vào hàm `findMatches` và sửa đổi/thêm điều kiện `if`.
- **Khắc phục:** Cần tách phần xử lý tạo kẹo đặc biệt ra khỏi hàm `findMatches` trong lớp `BasicMatchLogic`.  
Thay vì viết nhiều điều kiện `if-else` trực tiếp trong `findMatches`, ta nên tạo các phương thức riêng để xử lý từng trường hợp như:

  - `createStripedCandy()` → tạo kẹo sọc khi match 4 viên
  - `createWrappedCandy()` → tạo kẹo gói nổ khi match từ 5 viên trở lên
  - `createSpecialCandyByMatchSize()` → kiểm tra kích cỡ match và gọi hàm tạo kẹo phù hợp

Khi đó, hàm `findMatches` chỉ tập trung vào việc tìm các viên kẹo trùng nhau, còn việc tạo kẹo đặc biệt sẽ được xử lý riêng.  
Nếu sau này cần thêm kiểu kẹo mới, chỉ cần thêm phương thức xử lý mới mà không làm thay đổi quá nhiều logic chính trong `findMatches`.
